...
mydb=connection.MySQLConnection(user='root',host='localhost',password='admin',db='ecom')
@app.route('/')
def home():
...
def myorders():
    if session.get('user'):
        try:
            ...
            return render_template('orders.html',myorder=myorder)
    else:
        flash('pls login first')
        return redirect(url_for('userlogin'))
